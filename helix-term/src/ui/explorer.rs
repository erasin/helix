...
impl Explorer {
    ...
    fn render_embed(
        ...
        let commandline = cx.editor.config().commandline as u16;
        let statusline = 1;
        ...
        // NOTE - statusline if hide commandline for
        let prompt_area = if commandline == 0 {
            area.clip_top(side_area.height - statusline)
        } else {
            area.clip_top(side_area.height)
        }
